<template lang="html">
	<div class="kjz">
		 <canvas class="canvasYun" :id="id" :style="{'width':sizeW+'rem','height':sizeH+'rem'}"></canvas>
	</div>
</template>	
<script>
//以下都是倍数
  ...
  methods:{
     canvasImg(){
        ...
        this.img.onload=()=>{
          ...
          this.cxt.drawImage(this.img,-0.5 * this.w, -.5 * this.h,this.w*this.width,this.h*this.height);
          setTimeout(()=>{this.imgD();},this.time)
        }; 
        
        
		},
		imgD(){   
       if(this.numOne>=150) this.numbol=false;
       if(this.numOne<=0) this.numbol=true;
