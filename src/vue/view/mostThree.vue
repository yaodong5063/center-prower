<template lang="html">
  <div class="mostIndex" :style="{height:$store.state.vLang.height+'rem'}">
     ...
     <div class="fontsDom">
       ...
       <p class="tip">{{lang.mostThree.text}}</p>
       <div class="button">
           <button @click="back">{{lang.mostThree.back}}</button>
       </div>
...
export default {
  ...
  data:function(){
    return {
        ...
        mostArr:!!localStorage.mostThree?JSON.parse(localStorage.mostThree):[],
        Arr:['A','B','C','D'],
        timeObj:{},
        timeObj1:{}
  ...
  methods:{
    mostAjax(){
      if(!!localStorage.mostThree) return !1;
      let obj={};
      obj.language=localStorage.lang=='en'?2:1;
      const mostArr=[...JSON.parse(localStorage.most),...JSON.parse(localStorage.mostTwo)];
      let arr=[];
      mostArr.map((item)=>{
         arr.push(item.pid);
      });
      ...
      getPoems(obj,(data)=>{
        if(data.code===200){
           ...
           this.mostArr=_data;
           localStorage.mostThree=JSON.stringify(_data);
        }else{
